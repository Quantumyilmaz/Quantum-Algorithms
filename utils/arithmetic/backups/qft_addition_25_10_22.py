...
from utils.misc import execute_circ
from qiskit import QuantumCircuit, QuantumRegister, ClassicalRegister

def qft_adder(add_this,add_to_this=0, n=10, to_gate=True, draw_barriers=False):
    a , b = add_to_this , add_this
    ...
    apply_qft_to_these = list(range(n))

    for i in list(np.where(binary_a[::-1])[0]):
        qc_.x(i)
    ...
    while counter:
        for i in range(counter):
            qc_.cp(2*np.pi/2**(i+1),2*n-counter+i, n-counter)
        if draw_barriers:
            qc_.barrier()
        counter -= 1
...
def qft_adder_to_integer(qc,cr=None):
    ...
    results_counts = execute_circ(circuit).get_counts()
...
# def qft_adder_v0(add_this,add_to_this=0, n=10, to_gate=True, draw_barriers=False):
#     a , b = add_to_this , add_this
#     binary_a,binary_b = [*map(int,np.binary_repr(a))], [*map(int,np.binary_repr(b))]
#     # assert n > max(len(binary_a), len(binary_b)), max(len(binary_a), len(binary_b))
#     assert n >= len(np.binary_repr(a+b)),len(np.binary_repr(a+b))
...
# def qft_adder_to_integer_v0(qc,cr=None):
